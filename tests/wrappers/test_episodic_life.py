...
from atarax.wrappers import EpisodicLifeState, EpisodicLife
...
def test_no_life_loss_not_terminal(fake_env):
    env = EpisodicLife(fake_env)
    _, state = env.reset(_key, _params)
    _, _, _, done, info = env.step(_key, state, _action, _params)
...
def test_life_loss_signals_terminal(fake_env_class):
    class _LifeLossEnv(fake_env_class):
        def step(self, rng, state, action, params):
            obs, ns, reward, _, info = super().step(rng, state, action, params)
