...
"""James Bond 007 — reward and terminal extraction, plus reset/step entry points."""
...
# Source: Arcade-Learning-Environment src/ale/games/supported/JamesBond.cpp
# Score: getDecimalScore(0xDC, 0xDD, 0xDE) — lower/middle/higher packed BCD bytes.
SCORE_LO = 0xDC  # ones and tens
SCORE_MID = 0xDD  # hundreds and thousands
SCORE_HI = 0xDE  # ten-thousands and hundred-thousands
LIVES_ADDR = 0x86  # low nibble holds lives; actual lives = (RAM[0x86] & 0xF) + 1
SCREEN_ADDR = 0x8C  # 0x68 on death/game-over screen


class JamesBond(AtariGame):
    """James Bond 007 game logic: reward and terminal extraction, reset, and step.

    Terminal when the lives nibble (RAM[0x86] & 0xF) equals 0 *and* RAM[0x8C]
    equals 0x68 (the death screen flag), matching ALE behaviour.
    """
    ...
    def is_terminal(self, ram: chex.Array, lives_prev: chex.Array) -> chex.Array:
        ...
        Terminal when the lives nibble equals 0 and RAM[0x8C] equals 0x68
        (death screen byte).
        ...
        return (lives_nibble == jnp.uint8(0)) & (ram[SCREEN_ADDR] == jnp.uint8(0x68))
