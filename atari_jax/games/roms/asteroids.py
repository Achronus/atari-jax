# Copyright 2026 Achronus
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#    http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
# ==============================================================================

"""Asteroids — reward and terminal extraction, plus reset/step entry points."""

import chex
import jax.numpy as jnp

from atari_jax.games.base import AtariGame

# RAM addresses (decimal indices into the 128-byte RIOT RAM array).
# Source: Arcade-Learning-Environment src/ale/games/supported/Asteroids.cpp
# Score: getDecimalScore(0xBE, 0xBD) * 10 — lower/higher packed BCD bytes.
# Score wraps at 100,000; negative deltas are corrected by adding 100,000.
SCORE_LO = 0xBE  # ones, tens, hundreds, thousands (lower two BCD digits)
SCORE_HI = 0xBD  # thousands and ten-thousands (higher two BCD digits)
LIVES_ADDR = 0xBC  # upper nibble holds lives remaining


class Asteroids(AtariGame):
    """Asteroids game logic: reward and terminal extraction, reset, and step."""

    _WRAP_SCORE = 100_000

    def _score(self, ram: chex.Array) -> chex.Array:
        """Decode the 4-digit packed-BCD score (× 10) from two RAM bytes."""
        lo = ram[SCORE_LO].astype(jnp.int32)
        hi = ram[SCORE_HI].astype(jnp.int32)
        return (
            jnp.int32(1000) * (hi >> 4)
            + jnp.int32(100) * (hi & 0xF)
            + jnp.int32(10) * (lo >> 4)
            + jnp.int32(1) * (lo & 0xF)
        ) * jnp.int32(10)

    def get_lives(self, ram: chex.Array) -> chex.Array:
        """
        Read the lives counter from RAM.

        Parameters
        ----------
        ram : chex.Array
            uint8[128] — RIOT RAM snapshot.

        Returns
        -------
        lives : chex.Array
            int32 — Lives remaining (upper nibble of 0xBC).
        """
        return ram[LIVES_ADDR].astype(jnp.int32) >> 4

    def get_reward(self, ram_prev: chex.Array, ram_curr: chex.Array) -> chex.Array:
        """
        Compute the reward earned in the last step as a score delta.

        Score wraps at 100,000; a negative delta is corrected by adding the
        wrap value to recover the true reward.

        Parameters
        ----------
        ram_prev : chex.Array
            uint8[128] — RIOT RAM before the step.
        ram_curr : chex.Array
            uint8[128] — RIOT RAM after the step.

        Returns
        -------
        reward : chex.Array
            float32 — Score gained this step (non-negative).
        """
        delta = self._score(ram_curr) - self._score(ram_prev)
        delta = jnp.where(delta < 0, delta + jnp.int32(self._WRAP_SCORE), delta)
        return delta.astype(jnp.float32)

    def is_terminal(self, ram: chex.Array, lives_prev: chex.Array) -> chex.Array:
        """
        Determine whether the episode has ended.

        Parameters
        ----------
        ram : chex.Array
            uint8[128] — RIOT RAM after the step.
        lives_prev : chex.Array
            int32 — Lives count from before the step.

        Returns
        -------
        terminal : chex.Array
            bool — True when the episode ended on this step.
        """
        return (lives_prev > jnp.int32(0)) & (self.get_lives(ram) == jnp.int32(0))
