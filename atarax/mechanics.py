...
def grid_remove_hit(
    ...
    """
    Remove cells from a boolean occupancy grid where `hit_mask` is True.
    ...
    grid : chex.Array
        bool[rows, cols] — Active cell flags.
    hit_mask : chex.Array
        bool[rows, cols] — Cells to deactivate (e.g. from `grid_hit_test`).

    Returns
    -------
    new_grid : chex.Array
        bool[rows, cols] — Grid with hit cells cleared.
    """
    return grid & ~hit_mask
