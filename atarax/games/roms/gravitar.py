...
"""Gravitar — reward and terminal extraction, plus reset/step entry points."""
...
# Source: Arcade-Learning-Environment src/ale/games/supported/Gravitar.cpp
# Score: getDecimalScore(0x09, 0x08, 0x07) — lower/middle/higher packed BCD bytes.
SCORE_LO = 0x09  # ones and tens
SCORE_MID = 0x08  # hundreds and thousands
SCORE_HI = 0x07  # ten-thousands and hundred-thousands
LIVES_ADDR = 0x84  # raw lives counter; actual lives = RAM[0x84] + 1
SCREEN_ADDR = 0x81  # current screen/state: 0x00 = title, 0x01 = game over


class Gravitar(AtariGame):
    """Gravitar game logic: reward and terminal extraction, reset, and step.

    Terminal detection uses RAM[0x81]: value 0x01 is the game-over screen.
    On the title screen (value 0x00) lives are pinned to 6 to avoid a false
    zero reading before the game initialises the lives counter.
    """
    ...
    def get_lives(self, ram: chex.Array) -> chex.Array:
        ...
        Returns 6 on the title screen (RAM[0x81] == 0x00) where the lives
        counter is not yet initialised.  Otherwise returns RAM[0x84] + 1.
        ...
        """
        on_title = ram[SCREEN_ADDR] == jnp.uint8(0x00)
    ...
    def is_terminal(self, ram: chex.Array, lives_prev: chex.Array) -> chex.Array:
        ...
        Terminal when RAM[0x81] equals 0x01 (game-over screen).
        ...
        """
        return ram[SCREEN_ADDR] == jnp.uint8(0x01)
