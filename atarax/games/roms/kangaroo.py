...
"""Kangaroo — reward and terminal extraction, plus reset/step entry points."""
...
# Source: Arcade-Learning-Environment src/ale/games/supported/Kangaroo.cpp
# Score: getDecimalScore(0xA8, 0xA7) * 100 — lower/higher 2-byte BCD × 100.
SCORE_LO = 0xA8  # ones and tens (of hundreds)
SCORE_HI = 0xA7  # hundreds and thousands (of hundreds)
LIVES_ADDR = 0xAD  # lower 3 bits hold lives − 1; 0xFF is the game-over sentinel


class Kangaroo(AtariGame):
    """Kangaroo game logic: reward and terminal extraction, reset, and step.

    Score is 2-byte packed BCD multiplied by 100.  Terminal when RAM[0xAD]
    equals 0xFF (underflow game-over sentinel).
    """
    ...
    def get_lives(self, ram: chex.Array) -> chex.Array:
        ...
        -------
        lives : chex.Array
            int32 — Lives remaining ((RAM[0xAD] & 0x7) + 1; 0 on sentinel 0xFF).
        """
        byte = ram[LIVES_ADDR]
        normal = (byte & jnp.uint8(0x07)).astype(jnp.int32) + jnp.int32(1)
        return jax.lax.select(byte == jnp.uint8(0xFF), jnp.int32(0), normal)
    ...
    def is_terminal(self, ram: chex.Array, lives_prev: chex.Array) -> chex.Array:
        ...
        Terminal when RAM[0xAD] equals 0xFF (underflow game-over sentinel).
