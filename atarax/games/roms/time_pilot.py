...
"""Time Pilot — reward and terminal extraction, plus reset/step entry points."""
...
# Source: Arcade-Learning-Environment src/ale/games/supported/TimePilot.cpp
# Score: getDecimalScore(0x8D, 0x8F) * 100 — lower/higher BCD × 100 (non-sequential).
SCORE_LO = 0x8D  # ones and tens (of hundreds)
SCORE_HI = 0x8F  # hundreds and thousands (of hundreds)
LIVES_ADDR = 0x8B  # lower 3 bits hold lives − 1
TERM_ADDR = 0xA0  # non-zero when game is over


class TimePilot(AtariGame):
    """Time Pilot game logic: reward and terminal extraction, reset, and step.

    Score is 2-byte packed BCD × 100 (non-sequential addresses).  Terminal when
    RAM[0xA0] is non-zero.
    """
    ...
    def get_lives(self, ram: chex.Array) -> chex.Array:
        ...
        -------
        lives : chex.Array
            int32 — Lives remaining ((RAM[0x8B] & 0x7) + 1).
        """
    ...
    def is_terminal(self, ram: chex.Array, lives_prev: chex.Array) -> chex.Array:
        ...
        Terminal when RAM[0xA0] is non-zero (game-over flag).
        ...
        """
        return ram[TERM_ADDR] != jnp.uint8(0)
