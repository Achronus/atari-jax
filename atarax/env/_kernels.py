...
All public functions here accept `rom` and `game_id` as **dynamic JAX
arguments** — not Python-level constants at the JIT boundary.  Consequently
every game with the same ROM size (4 KB) and the same `frame_skip` /
`max_episode_steps` values shares a **single XLA compilation**, reducing
first-run compile time from O(57) to O(1).
...
def jit_rollout(
    ...
    def _step(carry_state, action):
        obs, new_state, reward, done, info = jit_step(
...
def jit_vec_rollout(
    ...
    def _step(carry_states, t_actions):
        screens, new_states, rewards, done, info = jit_vec_step(
