...
DEFAULT_CACHE_DIR = pathlib.Path(
    os.environ.get(
        "ATARAX_CACHE_DIR",
        str(pathlib.Path.home() / ".cache" / "atarax" / "xla_cache"),
    )
)
...
def setup_cache(cache_dir: pathlib.Path | str | None = DEFAULT_CACHE_DIR) -> None:
    ...
    cache_dir : Path | str | None
        ...
        at `{cache_dir}/{backend}/`.  Defaults to the `ATARAX_CACHE_DIR`
        environment variable, or `~/.cache/atarax/xla_cache` if unset.
